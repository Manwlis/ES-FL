...
protected:
    ...
    // buffer instances
    accel_softmax_weights_RAM_AUTO_1R1W_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(accel_softmax_weights_RAM_AUTO_1R1W) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "accel_softmax_weights_RAM_AUTO_1R1W_memcore_U_";
            name += szTmp;

            buffer[i] = new accel_softmax_weights_RAM_AUTO_1R1W_memcore(name.c_str());
            buffer[i]->clk      ( clk );
