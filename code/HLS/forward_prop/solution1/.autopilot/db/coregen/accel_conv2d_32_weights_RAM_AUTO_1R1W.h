...
#ifndef _accel_conv2d_32_weights_RAM_AUTO_1R1W_H_
#define _accel_conv2d_32_weights_RAM_AUTO_1R1W_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "accel_conv2d_32_weights_RAM_AUTO_1R1W_memcore.h"

SC_MODULE(accel_conv2d_32_weights_RAM_AUTO_1R1W) {
...
protected:
    ...
    // buffer instances
    accel_conv2d_32_weights_RAM_AUTO_1R1W_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(accel_conv2d_32_weights_RAM_AUTO_1R1W) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "accel_conv2d_32_weights_RAM_AUTO_1R1W_memcore_U_";
            name += szTmp;

            buffer[i] = new accel_conv2d_32_weights_RAM_AUTO_1R1W_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "accel_conv2d_32_weights_RAM_AUTO_1R1W.trace");
            // ports
