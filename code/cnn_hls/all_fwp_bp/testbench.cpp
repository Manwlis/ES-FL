...
int main ( int argc , char** argv )
{
	...
	static float conv_32_weights_fp[conv_32_in_channels][conv_32_filter_height][conv_32_filter_width][conv_32_num_filters];
	static float conv_32_weights_bp[conv_32_num_filters][conv_32_filter_height][conv_32_filter_width][conv_32_in_channels];
	...
	for( int y = 0 ; y < conv_32_filter_height ; y++ )
		for ( int x = 0 ; x < conv_32_filter_width ; x++ )
			for ( int channel = 0 ; channel < conv_32_in_channels ; channel++ )
				...
				{
					getline( file , line );
					conv_32_weights_fp[channel][y][x][kernel] = std::stof(line);
					conv_32_weights_bp[kernel][y][x][channel] = std::stof(line);
				}
	file.close();
	...
	/*********** Print Results ***********/
	save_array < maxp_16_in_height * maxp_16_in_width * maxp_16_in_channels > ( maxp_16_in_error , "output_gradients/l0_conv16_hls.txt" , 4 );
}
