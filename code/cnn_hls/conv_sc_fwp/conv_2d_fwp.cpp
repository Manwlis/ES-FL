...
void conv2d_sum (
	...
	float gmem_weights[num_filters][filter_height][filter_width] ,
...
#pragma HLS PIPELINE II=16
		...
		filter: for ( unsigned int filter = 0 ; filter < num_filters ; filter++ )
		{
			...
			// this can be replaced with a vector to reduce axi read requests. Requires double resources.
			float temp_weights[filter_height][filter_width];
			memcpy( temp_weights , &gmem_weights[filter] , filter_height * filter_width * sizeof(float) );

			filt_y: for ( unsigned int filter_y = 0 ; filter_y < filter_height ; filter_y++ )
				filt_x: for ( unsigned int filter_x = 0 ; filter_x < filter_width ; filter_x++ )
					temp_sum += temp_window.elements[filter_y][filter_x] * temp_weights[filter_y][filter_x];
...
void accel (
	axis_float_stream& input_stream ,
	float conv2d_16_weights[conv2d_16_num_filters][conv2d_16_filter_height][conv2d_16_filter_width] ,
	float conv2d_16_biases[conv2d_16_num_filters] ,
	float conv2d_16_feature_map[conv2d_16_out_height * conv2d_16_out_width * conv2d_16_num_filters] )
{
#pragma HLS DATAFLOW
#pragma HLS INTERFACE mode=m_axi port=conv2d_16_biases bundle=gmem1
#pragma HLS INTERFACE mode=m_axi port=conv2d_16_weights bundle=gmem2
	// data streams
	hls::stream < window < float , conv2d_16_filter_height , conv2d_16_filter_width > , 2 > conv2d_16_window_stream;
	hls::stream < window < float , conv2d_16_filter_height , conv2d_16_filter_width > , 2 > conv2d_16_padded_window_stream;
	hls::stream < float , 2 > kernel_sums;

	// output streams
	hls::stream < float , 2 > feature_map_stream;
	hls::stream < bool , conv2d_16_out_height * conv2d_16_out_width * conv2d_16_num_filters > activations_stream;

	// create windows
	create_window_stream_conv2d < float , conv2d_16_in_height , conv2d_16_in_width ,
		conv2d_16_filter_height , conv2d_16_filter_width >
		( input_stream , conv2d_16_window_stream );

	pad_windows < float , conv2d_16_in_height , conv2d_16_in_width ,
		conv2d_16_filter_height , conv2d_16_filter_width , conv2d_16_num_filters >
		( conv2d_16_window_stream , conv2d_16_padded_window_stream );

	// full convolution
	conv2d_sum < float , conv2d_16_in_height , conv2d_16_in_width ,
		conv2d_16_filter_height , conv2d_16_filter_width , conv2d_16_num_filters >
		( conv2d_16_padded_window_stream , conv2d_16_weights , conv2d_16_biases , kernel_sums );

	conv2d_activate < conv2d_16_in_height , conv2d_16_in_width ,
		conv2d_16_filter_height , conv2d_16_filter_width , conv2d_16_num_filters >
		( kernel_sums , feature_map_stream , activations_stream );

	// write in memory
	write_mem_wrapper <
		float , bool , conv2d_16_out_height * conv2d_16_out_width * conv2d_16_num_filters >
		( feature_map_stream , conv2d_16_feature_map , activations_stream );
}
