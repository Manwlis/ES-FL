...
void maxi_labels_to_stream ( uint batch ,
	t_label input_labels[c_num_batches][batch_size] , hls::stream < t_label >& s_input )
{
	stream_labels:
...
//	hls::stream < window_1d < float , maxi_buffer_size > ,  1 > s_input_cg_buffered ("s_input_cg_buffered");
	...
	hls::stream <                                t_label , 8 > s_labels ("s_labels");
...
/***** layer 1, 14x14x16 -> maxp 2x2 backprop -> 28x28x16 *****/
	...
	maxp_bp < batch_size , l1_maxp_in_h , l1_maxp_in_w , l1_maxp_in_c , l1_maxp_f_h , l1_maxp_f_w , l1_maxp_f_st , 3 >
...
#pragma HLS STABLE variable=l5_soft_bias_grad
	...
	update_l0_conv_weights ( batch_lr , s_l0_conv_weight_grad , l0_conv_weights );
	update_l0_conv_biases ( batch_lr , s_l0_conv_bias_grad , l0_conv_biases );

	update_l2_conv_weights ( batch_lr , s_l2_conv_weight_grad , l2_conv_weights_fp , l2_conv_weights_bp );
	update_l2_conv_biases ( batch_lr , s_l2_conv_bias_grad , l2_conv_biases );
...
#pragma HLS INTERFACE mode=m_axi port=gmem_input_data_cg bundle=gmem2
	...
	float l5_soft_weight_grad[l5_soft_in_size][l5_soft_k]; // keep those as arrays so >1 elements can be accessed per cycle.
...
#pragma HLS ARRAY_PARTITION variable=l5_soft_weight_grad dim=2 type=complete
	...
	for ( uint num_batch = 0 ; num_batch < c_num_batches ; num_batch++ )
	{
		process_batch ( gmem_input_data_fp , gmem_input_data_cg , gmem_labels , num_batch ,
			...
			s_l2_conv_weight_grad , s_l2_conv_bias_grad , s_l0_conv_weight_grad , s_l0_conv_bias_grad );
