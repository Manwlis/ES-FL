...
#pragma HLS PIPELINE II=12 style=frp
					...
					in_c: for ( uint in_c = 0 ; in_c < _in_c ; in_c++ )
					{
						...
						filt_y: for ( uint filter_y = 0 ; filter_y < _f_h ; filter_y++ )
							filt_x: for ( uint filter_x = 0 ; filter_x < _f_w ; filter_x++ )
								channel_errors.elements[in_c] += temp_window.elements[_f_h - 1 - filter_y][_f_w - 1 - filter_x]
																										   * weights[filter_y][filter_x][in_c][filter];
					}
	...
	float weight_grads[_f_h][_f_w][_num_f] , float bias_grads[_num_f] )
{
	...
	batch: for ( uint batch = 0 ; batch < _batch_size ; batch++ )
		in_y: for( uint in_y = 0 ; in_y < _in_h ; in_y++ )
			in_x: for( uint in_x = 0 ; in_x < _in_w ; in_x++ )
			{
				// reads and pipeline in_x loop and, s_kernel_error group it?
				out_f: for ( uint filter = 0 ; filter < _num_f ; filter++ )
				{
#pragma HLS PIPELINE II=3 style=frp
					if ( filter == 0 )
