...
void server_to_client_msg_little_endianess( Server_to_client_msg& message )
{
	...
	// flip floats in variables array
	for( int i = 0  ; i < VARIABLES_NUM ; i++ )
		message.variables[i] = Utils::reverse_float( message.variables[i] );
}
...
void client_to_server_msg_big_endianess( Client_to_server_msg& message )
{
	...
	for( int i = 0  ; i < VARIABLES_NUM ; i++ )
		message.variables[i] = Utils::reverse_float( message.variables[i] );
	// flip stats floats
	message.accuracy = Utils::reverse_float( message.accuracy );
	message.loss = Utils::reverse_float( message.loss );
}
